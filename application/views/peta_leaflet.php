...
div.innerHTML = '<img src="<?=base_url()?>assets/arah-mata-angin.png"style=width:200px;>';return div; } 
north.addTo(map);

$.getJSON("<?=base_url()?>assets/provinsi.geojson",function(data){ 
    var ratIcon = L.icon({ 
        iconUrl: '<?=base_url()?>assets/tanda.png', 
        iconSize: [12,10] 
}); 
    L.geoJson(data,{ 
        pointToLayer: function(feature,latlng){ 
            var marker = L.marker(latlng,{icon: ratIcon}); 
            marker.bindPopup(feature.properties.ADMIN_NAME);
            return marker; 
        } 
    }).addTo(prov); 
});
...
$.getJSON("<?=base_url()?>assets/poliklinik.geojson",function(data){ 
        var ratIcon = L.icon({ 
        iconUrl: '<?=base_url()?>assets/tanda1.png', 
        iconSize: [12,10] 
}); 
L.geoJson(data,{ 
        pointToLayer: function(feature,latlng){ 
...
$.getJSON("<?=base_url()?>assets/puskesmas.geojson",function(data){ 
        var ratIcon = L.icon({
        iconUrl: '<?=base_url()?>assets/tanda2.png', 
        iconSize: [12,10] 
}); 
L.geoJson(data,{ 
        pointToLayer: function(feature,latlng){ 
...
$.getJSON("<?=base_url()?>/assets/sungai.geojson",function(kode){ 
        ...
        return { color: "#999", weight: 3, color: color, fillOpacity: .4 }; 
 }, 
...
$.getJSON("<?=base_url()?>/assets/provin.geojson",function(kode){
        L.geoJson( kode, {
        style: function(feature){
        var fillColor,
            kode = feature.properties.kode;
